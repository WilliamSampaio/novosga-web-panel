<template>
  <v-dialog max-width="600" transition="dialog-top-transition">
    <template v-slot:activator="{ props: activatorProps }">
      <v-btn v-bind="activatorProps" color="success" text="Carregar da pasta custom" variant="flat"></v-btn>
    </template>

    <template v-slot:default="{ isActive }">
      <v-card title="Carregar imagem da pasta custom"
...
onMounted(() => {
  const modulos = import.meta.glob('/public/images/custom/*.{png,jpg,jpeg,svg,webp}', { eager: true })
